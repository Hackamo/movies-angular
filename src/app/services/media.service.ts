...
export class MediaService {
	...
	getDetails(type: 'movie' | 'serie', id: string): Observable<any> {
		...
		return this.httpClient.get<any>(`${this.api_url}${endpoint}${id}?api_key=${this.api_key}&language=${this.language}`)
	}
	...
	getPopular(type: 'movie' | 'serie', pagination: number): Observable<any> {
		...
		return this.httpClient.get<any>(this.api_url + endpoint + '&page=' + pagination + '&language=' + this.language)
	}
	...
	searchMedia(type: 'movie' | 'serie', query: string, pagination: number): Observable<any> {
		return type === 'movie' ? this.searchMovies(query, pagination) : this.searchSeries(query, pagination)
	}

	getReleaseMedia(type: 'movie' | 'serie', pagination: number): Observable<any> {
		const endpoint = type === 'movie' ? this.movie_release : this.serie_release
		return this.httpClient.get<any>(this.api_url + endpoint + pagination + '&language=' + this.language)
	}
